...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
  ...
  if (create_cell->handshake_type != ONION_HANDSHAKE_TYPE_FAST) {
    ...
    len = onion_skin_server_handshake(
        ...
        create_cell->handshake_len, NULL, NULL, created_cell.reply,
        sizeof(created_cell.reply), keys, CPATH_KEY_MATERIAL_LEN,
        rend_circ_nonce, &params);
    tor_free(create_cell);
    if (len < 0) {
  ...
  if (cell->command == CELL_RELAY_EARLY) {
    if (direction == CELL_DIRECTION_IN) {
      ...
      if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        log_warn(LD_OR, " upstream=%s", channel_describe_peer(circ->n_chan));
      }
      circuit_mark_for_close(circ, END_CIRC_REASON_TORPROTOCOL);
      return;
    } else {
      ...
      if (or_circ->remaining_relay_early_cells == 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR,
               ...
               "  Closing circuit.",
               (unsigned)cell->circ_id, safe_str(channel_describe_peer(chan)));
        circuit_mark_for_close(circ, END_CIRC_REASON_TORPROTOCOL);
        return;
      }
