           ...
           const char *filename, int lineno))
{
  ...
  if (ALL_SENDERS_DECIDE_DELAYS || cpath_layer != NULL) {
    cell.command = crypto_rand_int_range(CELL_RELAY_DELAY_LOWEST,
                                         CELL_RELAY_DELAY_HIGHEST);
    log_info(LD_GENERAL, "[RENDEZMIX,SEND,DELAY] (cmd=%d) {{%s}}",
             cell.command, payload);
  } else {
    log_info(LD_GENERAL, "[RENDEZMIX,SEND,NORMAL] (cmd=%d) {{%s}}",
             cell.command, payload);
  }
  if (CIRCUIT_IS_ORIGIN(circ)) {
...
relay_consensus_has_changed(const networkstatus_t *ns)
{
  ...
  max_circuit_cell_queue_size = get_param_max_circuit_cell_queue_size(ns);
