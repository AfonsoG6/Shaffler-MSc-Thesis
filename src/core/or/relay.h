...
uint32_t
relay_get_param_max_circuit_cell_queue_size(const networkstatus_t *ns);
...
MOCK_DECL(int, relay_send_command_from_edge_,
...
int connection_edge_package_raw_inbuf(edge_connection_t *conn,
                                      int package_partial, int *max_cells);
...
void destroy_cell_queue_append(destroy_cell_queue_t *queue, circid_t circid,
                               uint8_t reason);

void channel_unlink_all_circuits(channel_t *chan, smartlist_t *detached_out);
MOCK_DECL(int, channel_flush_from_first_active_circuit,
          (channel_t * chan, int max));
MOCK_DECL(int, circuit_package_relay_cell,
...
#ifdef RELAY_PRIVATE
STATIC int handle_relay_cell_command(cell_t *cell, circuit_t *circ,
...
STATIC size_t connection_edge_get_inbuf_bytes_to_package(
    size_t n_available, int package_partial, circuit_t *on_circuit);
