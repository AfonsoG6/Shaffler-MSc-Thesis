                                       ...
                                       extend_info_t *extend_info)
{
  ...
  // extend info.
  return extend_info_has_orport(extend_info, &TO_CONN(tlschan->conn)->addr,
...
channel_tls_handle_cell(cell_t *cell, or_connection_t *conn)
{
  ...
  case CELL_CREATED2:
    ...
    if (cell->command == CELL_RELAY)
      log_info(LD_GENERAL, "[RENDEZMIX,RECEIVED,NORMAL]");
  ...
  default:
    ...
    if (cell->command >= CELL_RELAY_DELAY_LOWEST &&
      ...
      if (n_chan) {
        ...
        if (n_conn)
          tor_inet_ntoa(&(n_conn->addr.addr.in_addr), next_node_addr, 64);
      }
      if (direction == CELL_DIRECTION_OUT) {
        log_info(LD_GENERAL,
                 "[RENDEZMIX,RECEIVED,DELAY] (cmd=%d) (p_addr=%s) (n_addr=%s)",
                 cell->command, prev_node_addr, next_node_addr);
        if (probably_middle_node(conn, circ)) {
          ...
          log_info(
              LD_GENERAL,
              "[RENDEZMIX,DELAYED] (cmd=%d) (ns=%ld) (p_addr=%s) (n_addr=%s)",
              cell->command, ts.tv_nsec, prev_node_addr, next_node_addr);
        ...
        log_info(
            LD_GENERAL,
            "[RENDEZMIX,RECEIVED,NORMAL] (cmd=%d) (p_addr=%s) (n_addr=%s)",
            cell->command, prev_node_addr, next_node_addr);
        cell->command = CELL_RELAY;
      }
      channel_process_cell(TLS_CHAN_TO_BASE(chan), cell);
...
channel_tls_process_versions_cell(var_cell_t *cell, channel_tls_t *chan)
{
  ...
  if (chan->conn->link_proto == 2) {
    log_info(LD_OR, "Negotiated version %d on %s; sending NETINFO.",
    ...
    log_info(
        LD_OR, "Negotiated version %d with on %s; %s%s%s%s%s",
        highest_supported_version, connection_describe(TO_CONN(chan->conn)),
...
mark_channel_tls_endpoint_as_client(channel_tls_t *chan)
{
  ...
  tor_assert(fast_mem_is_zero(
      (const char *)(chan->conn->handshake_state->authenticated_ed25519_peer_id
                         .pubkey),
      32));
...
channel_tls_process_netinfo_cell(cell_t *cell, channel_tls_t *chan)
{
  ...
  if (me && !TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer &&
    ...
    const char *descr = channel_describe_peer(TLS_CHAN_TO_BASE(chan));
    log_info(LD_OR,
...
#define ERR(s)                                             \
  do {                                                     \
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,                 \
           ...
           connection_describe(TO_CONN(chan->conn)), (s)); \
    connection_or_close_for_error(chan->conn, 0);          \
    goto err;                                              \
  } while (0)
...
#define ERR(s)                                             \
  ...
  if (use_type >= 0) {
    log_info(LD_OR,
             ...
             connection_describe(TO_CONN(chan->conn)), use_type);
...
#define ERR(s)                                             \
  ...
  chan->conn->handshake_state->digest_received_data = 0;
  {
    ...
    log_debug(LD_HANDSHAKE,
              ...
              connection_describe(TO_CONN(chan->conn)), __func__,
    ...
    log_debug(LD_HANDSHAKE,
              ...
              connection_describe(TO_CONN(chan->conn)), authtype);
  }
