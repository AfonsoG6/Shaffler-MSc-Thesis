...
def preprocessing_new_test_data(win, number_of_interval, flow, tor_len, exit_len):
    ...
    for i in range(0, number_of_traces):
        tor_seq[i] = [float(pair["ipd"]) * 1000.0 for pair in tor_seq[i]] + [float(pair["size"]) / 1000.0 for pair in tor_seq[i]]
        if len(tor_seq[i]) < (tor_len * 2):
            tor_seq[i] = tor_seq[i] + ([0] * ((tor_len * 2) - (len(tor_seq[i]))))
        elif len(tor_seq[i]) > (tor_len * 2):
            tor_seq[i] = tor_seq[i][0 : (tor_len * 2)]

        exit_seq[i] = [float(pair["ipd"]) * 1000.0 for pair in exit_seq[i]] + [float(pair["size"]) / 1000.0 for pair in exit_seq[i]]
        if len(exit_seq[i]) < (exit_len * 2):
            exit_seq[i] = exit_seq[i] + ([0] * ((exit_len * 2) - (len(exit_seq[i]))))
...
def eval_model(full_or_half, minimum_windows_positive, use_new_data, model1_path, model2_path, test_path, thr, use_global, muti_output_list, flow, tor_len, exit_len, win_interval, num_windows):
    global total_emb
    global total_vot
    global total_cos
    ...
    for win in range(num_windows):
        ...
        if use_new_data == 1:
            (test_data_tor, test_data_exit) = preprocessing_new_test_data(win, win_interval, flow, tor_len, exit_len)
        elif use_new_data != 1:
        ...
        if win in activated_windows:
            Cosine_Similarity_eval(tor_embs, exit_embs, threshold_result, single_output, win, last_activated_window, correlated_threshold_value, cosine_similarity_table, muti_output_list, flow)
