...
def create_model(input_shape=None, emb_size=None, model_name=""):
    ...
    model = Conv1D(
        ...
        padding="same",
        name="block1_conv1" + "_" + model_name,
    )(input_data)
    ...
    model = Conv1D(
        ...
        strides=conv_stride_size[1],
        padding="same",
        name="block1_conv2" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        padding="same",
        name="block2_conv1" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        strides=conv_stride_size[2],
        padding="same",
        name="block2_conv2" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        padding="same",
        name="block3_conv1" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        strides=conv_stride_size[3],
        padding="same",
        name="block3_conv2" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        padding="same",
        name="block4_conv1" + "_" + model_name,
    )(model)
    ...
    model = Conv1D(
        ...
        strides=conv_stride_size[4],
        padding="same",
        name="block4_conv2" + "_" + model_name,
    )(model)
    ...
    model = MaxPooling1D(
        ...
        padding="same",
        name="block4_pool" + "_" + model_name,
    )(model)

    output = Flatten()(model)

    dense_layer = Dense(emb_size, name="FeaturesVec" +
                        "_" + model_name)(output)

    shared_conv2 = Model(
        inputs=input_data, outputs=dense_layer, name=model_name)
    return shared_conv2
...
def create_model_2d(input_shape=None, emb_size=None):
    ...
    model = MaxPooling2D(pool_size=(1, 5), strides=(1, 1), padding="valid")(
        model
    )  # (None, 2, 338, 2000)
    model = Conv2D(
        32,
    ...
    # Issue: OOM when allocating tensor with shape[650000,3000]
    model = Dense(
        1024, activation="relu", kernel_initializer=RandomNormal(stddev=0.01, mean=0.0)
    )(model)
    model = Dropout(rate=0.6)(model)
    model = Dense(
        800, activation="relu", kernel_initializer=RandomNormal(stddev=0.01, mean=0.0)
    )(model)
    model = Dropout(rate=0.6)(model)
    model = Dense(
        100, activation="relu", kernel_initializer=RandomNormal(stddev=0.01, mean=0.0)
    )(model)
    model = Dropout(rate=0.6)(model)
    model = Dense(
        emb_size,
        activation="linear",
        kernel_initializer=RandomNormal(stddev=0.01, mean=0.0),
    )(model)
