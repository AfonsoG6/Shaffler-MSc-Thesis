...
def get_params():
    parser.add_argument("--tor_len", "-t", required=False, default=500)
    parser.add_argument("--exit_len", "-e", required=False, default=800)
    parser.add_argument("--win_interval", "-i", required=False, default=5)
...
def load_whole_seq_new(tor_seq, exit_seq, circuit_labels, test_c, train_c, model_gb):
    ...
    return (
        ...
        np.array(test_window1),
        np.array(test_window2),
        np.array(test_window1),
        np.array(test_window2),
    )
...
if __name__ == "__main__":
    ...
    def build_negatives(anc_idxs, pos_idxs, similarities, neg_imgs_idx, num_retries=50):
        ...
        for anc_idx, pos_idx in zip(anc_idxs, pos_idxs):
            ...
            # .difference(np.array([anchor_class]))
    ...
    gen_hard = SemiHardTripletGenerator(
        train_windows1,
        train_windows2,
        batch_size,
        all_traces_train_idx,
        train_windows1,
        train_windows2,
        None,
        None,
    ...
    for epoch in range(nb_epochs):
        ...
        gen_hard_odd = SemiHardTripletGenerator(
            train_windows1[X1],
            train_windows2[X1],
            batch_size,
            X2,
        ...
        gen_hard_even = SemiHardTripletGenerator(
            train_windows1[X2],
            train_windows2[X2],
            batch_size,
            X1,
