...
def parse_csv(csv_path, interval, file_names):  # option: 'sonly', 'tonly', 'both'
            ...
            for line in lines:
                ...
                if abs(size) > 512:  # ignore ack packets
                    ...
                    if len(big_pkt) != 0:
                        ...
                        here_seq.append(
                            {"ipd": last_pkt["ipd"], "size": sum(big_pkt)+big_pkt[0]})
                        big_pkt = []
                        num_here_big_pkt += 1
                    ...
                    here_seq.append(
                        {"ipd": time_size[0], "size": time_size[1]})
                    pre_h_time = time
    ...
    print(interval, 'mean', np.mean(np.array(here_len)), np.mean(np.array(there_len)),
          np.mean(num_here_big_pkt_cnt), np.mean(num_there_big_pkt_cnt), flow_cnt)
    print(interval, 'median', np.median(np.array(here_len)), np.median(np.array(
        there_len)), np.median(num_here_big_pkt_cnt), np.median(num_there_big_pkt_cnt), flow_cnt)
...
def create_overlap_window_csv(csv_path, file_list, prefix_pickle_output, interval, num_windows, addnum):
    ...
    for win in range(num_windows):
        here, there, labels = parse_csv(
            csv_path, [win*addnum, win*addnum+interval], file_names)
        ...
        with open(prefix_pickle_output + str(interval) + '_win' + str(win) + '_addn' + str(addnum) + '_w_superpkt.pickle', 'wb') as handle:
            pickle.dump({'tor': here, 'exit': there, "label": labels},
...
if __name__ == '__main__':
    ...
    create_overlap_window_csv(
        data_path, file_list_path, prefix_pickle_output, interval, windows, addnum)
