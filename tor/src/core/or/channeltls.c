...
void delay_cell(circuit_t *circ, channel_tls_t *chan, cell_t *cell)
{
  ...
  log_info(LD_GENERAL, "[RENDEZMIX][DELAY][%s] circ %u-%u cmd=%d", get_direction_str(get_direction(circ, chan, cell)), TO_OR_CIRCUIT(circ)->p_circ_id, circ->n_circ_id, circ->delay_command);
