                                    ...
                                    char *buf, size_t buflen)
{
  ...
  safe_or_connection_get_tls_desc(TO_SAFE_OR_CONN(conn->safe_conn), tls_state, sizeof(tls_state));
...
void
connection_or_process_event(event_label_t label, event_data_t data,
                            void *context)
{
  or_connection_t *or_conn = context;

  if (label == safe_or_conn_tcp_connecting_ev) {
    // TODO: anything to do here?
  } else if (label == safe_or_conn_tls_handshaking_ev) {
    connection_or_change_state(or_conn, OR_CONN_STATE_TLS_HANDSHAKING);
  } else if (label == safe_or_conn_link_handshaking_ev) {
    link_handshaking_ev_data *handshake_data = data.ptr;
    or_conn->tls_own_cert = handshake_data->tls_own_cert;
    ...
    if (or_conn->is_outgoing) {
      ...
      connection_or_change_state(or_conn,
          OR_CONN_STATE_TLS_SERVER_RENEGOTIATING);
      // TODO: should we choose a different state here?
      //       enter_v3_handshake_with_cell() assumes we're in state
      //       'OR_CONN_STATE_TLS_SERVER_RENEGOTIATING'
    }
  } else if (label == safe_or_conn_open_ev) {
    // TODO: anything to do here?
  } else if (label == safe_or_conn_closed_ev) {
    // TODO: improve the error handling below
...
connection_or_finished_connecting(or_connection_t *or_conn)
{
  ...
  if (proxy_type != PROXY_NONE) {
    log_warn(LD_BUG, "Connection proxies aren't yet supported by safe connections");
    tor_assert(0);
                                          ...
                                          const or_options_t *options)
{
  ...
  safe_or_connection_adjust_buckets(TO_SAFE_OR_CONN(TO_CONN(conn)->safe_conn),
                                    rate, burst, reset,
                        ...
                        channel_tls_t *chan))
{
  ...
  // we don't support proxy connections yet
...
connection_or_nonopen_was_started_here(or_connection_t *conn)
{
  ...
  return conn->is_outgoing; // hopefully this is the same as the above
}
...
connection_or_send_certs_cell(or_connection_t *conn)
{
  ...
  if (conn_in_server_mode) {
    ...
    own_link_cert = conn->tls_own_cert;
  }
  tor_assert(id_cert);
                                      ...
                                      int server)
{
...

  {
    ...
    if (server) {
      ...
      cert = conn->tls_own_cert;
    } else {
      //cert = tor_tls_get_peer_cert(conn->tls);
      cert = conn->tls_peer_cert;
    }
    if (!cert) {
  ...
  if (old_tlssecrets_algorithm) {
    ...
    if (safe_or_connection_tls_secrets(TO_SAFE_OR_CONN(safe_conn),
                                       auth->tlssecrets) < 0) {
    ...
    //                                    label);
    int r = safe_or_connection_key_material(TO_SAFE_OR_CONN(safe_conn),
                                            auth->tlssecrets,
                                            auth->cid, sizeof(auth->cid),
                                            label);
    if (r < 0) {
