...
def do_request(
    session: HTMLSession, address: str, headers: dict, timeout: float, socks5: str
):
...
def run_client(start_delay, configs):
    ...
    with HTMLSession() as session:
        while True:
            try:
                ...
                print(f"[COVER] Sending request to {address}")
                do_request(session, address, headers, timeout, socks5)
                fails = 0
                delta = time.time() - t0
                ...
                if tolerated_fails >= 0 and fails > tolerated_fails:
                    print("[COVER] Exceeded fail limit... Stopping Client Thread")
                    return
...
def main(configs):
    ...
    for i in range(num_threads):
        ...
        client_thread = Thread(target=run_client, args=[start_delay, configs])
        threads.append(client_thread)
        client_thread.daemon = True
        client_thread.start()
        print(f"[COVER] Launched new client thread with start delay {start_delay}")
    for thread in threads:
        thread.join()
        print("[COVER] Client thread has been joined")
    print("[COVER] All client threads have been joined... Exiting")
